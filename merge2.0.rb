...
ftp.login('fileshare@marketingtechfileshare.com','mX0dY8yE2zwS5n')
...
data.each do |rec|
	...
	if  rec[:"Location ID"] == "1090" ||  rec[:"Location ID"] == "1091" || rec[:"Location ID"] == "2092" then next end
...
report.sort.each do |location|
	...
	uniq_customers = location[1].uniq { |record| record[:"Customer Number"] }
	uniq_invoices = location[1].uniq { |record| record[:"PURL"] }  
	uniq_email = location[1].uniq { |record| record[:"Email"] }.delete_if{|e| !e.Email.include?("@")}
	no_email = location[1].uniq { |record| record[:"PURL"] }.delete_if{|e| !["#1",""].include?(e.Email.strip)}
	not_participate =location[1].uniq { |record| record[:"PURL"] }.delete_if{|e| !["#2","#3"].include?(e.Email.strip)}
	lr = []
	lr << location[0]
